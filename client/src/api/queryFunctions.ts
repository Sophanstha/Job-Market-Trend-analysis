import {type AnalyticsResponse, type CompareResponse, type HistoryItem, type SearchResponse } from "../types";
...
export const compareJobsFn = async(a:string , b:string):Promise<CompareResponse>=>{
const {data}= await api.get<CompareResponse>(`search/compare?a=${encodeURIComponent(a)}&b=${encodeURIComponent(b)}`)
return data
}
