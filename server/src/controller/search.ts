...
import { forecastDemand } from "../algorithms/forcast.ts";

export const search = async (req: AuthRequest, res: Response) => {
  try {
    const { query } = req.body;

    // 1. Load all jobs from MongoDB
    const allJobs = await JobData.find({});
    if (allJobs.length === 0) {
      res.status(404).json({
        message: "No job data found. Please seed the database.",
      });
      return;
    }
    ...
    // 5. Calculate forecast to 2030
    const forecast = forecastDemand(
      topResult.historicalDemand,
      topResult.years,
      topResult.growthRate,
    );
...
export const compare = async (
  ...
  try {
    ...
    if (!a || !b) {
      res.status(400).json({
        message:
          "Provide two jobs: ?a=ai&b=cloud or ?a=software engineer&b=data scientist",
      });
      return;
    }
    ...
    const higherRemote =
      jobA.remoteAvailability >= jobB.remoteAvailability ? "A" : "B";
    ...
    res.status(200).json({
      ...
      comparison: {
        ...
        differences: {
          ...
          remoteDiff: Math.abs(
            jobA.remoteAvailability - jobB.remoteAvailability,
          ),
        },
      },
    });
